...
class NCCamera {
  ...
  startLive = (event) => {
    ...
    if (this.camera === null) {
      console.warn('NCCamera', 'No camera found, please check camera and call setup');
      return;
    }
  ...
  runLive = () => {
    ...
    this.camera.takePicture({
      ...
      if (er < 0) {
        console.warn('NCCamera', 'Error received from camera, stopping live', er);
        this.stopLive();
        this.hotplug();
      }
      else {
  ...
  autofocus = () => {
    return this.setSetting('cancelautofocus', 1)
      .then(this.setSetting('autofocusdrive', 1));
  }
